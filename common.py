...
def save_model(model: Any, target: str, output_dir: str):
    ...
    torch.save(model.state_dict(), f'{output_dir}\\{target}.pth')
...
def load_model(trained_model_dir: str, target_column_name: str) -> Any:
    ...
    # print(trained_model_dir, target_column_name)
    # print(torch.load(f"{trained_model_dir}\\{target_column_name}.pth"))
    model = models.mobilenet_v3_small(pretrained=True)  # Substitute 'your_model_architecture' with your actual model's architecture
    ...
    model_path = os.path.join(trained_model_dir, f"{target_column_name}.pth")
    ...
    return model
    # return torch.load(f"{trained_model_dir}\\{target_column_name}.pth")
